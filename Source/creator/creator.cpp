...
void Creator::createLlrxFile(const std::string &file) {
  /// MidiFile instance. Using to std::unique pointer for local instance and self
  /// desturction.
  ...
  const auto juceFile =
      File::getCurrentWorkingDirectory().getChildFile(juce::String((file).c_str()));
  ...
  const auto llrx_file =
      std::string(__LLRX_OUTPUT__) + "/" + replace_extension(file, ".llrx");
  ...
  if (validateLlrx(llrx_file, amount))
    std::cout << "###### " << llrx_file << " created and verified!" << std::endl
              << std::endl;
  else {
    std::cout << "###### " << file + ".llrx failed... could not verify " << llrx_file
              << " :[" << std::endl;
    std::filesystem::remove(llrx_file);
  }
}

EntityVector Creator::createEntities(flatbuffers::FlatBufferBuilder &builder,
                                     std::unique_ptr<MidiFile> &midi_file) {
  /// Vector that holds all the entities
  EntityVector vector;

  /// Interate through all the tracks in the midi file and add all `noteOn` messages to
  /// the enenties vector
  for (int i = 0; i < midi_file->getNumTracks(); i++) {
    ...
    for (int j = 0; j < track->getNumEvents(); j++) {
      ...
      /// When the note is on, we will store that information in an entity and add it to
      /// the enitites vector
...
LlrxFile Creator::createLlrx(flatbuffers::FlatBufferBuilder &builder,
                             const std::string &name, const int32_t &amount,
...
bool Creator::validateLlrx(const std::string &llrx, const int32_t entity_amount) {
  ...
  flatbuffers::Verifier verifier(reinterpret_cast<const uint8_t *>(readbuf.c_str()),
                                 readbuf.length());
  ...
  return VerifyLlrxBuffer(verifier) &&
         entity_amount == GetLlrx(readbuf.c_str())->amount();
}
